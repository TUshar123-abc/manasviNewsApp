...
const News = (props) => {
  ...
  const updateNews = useCallback(async () => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=763d45d8c3a14c8ba7bcb8fb6c8c6766&page=${page}&pageSize=${props.pageSize}`;
    setLoading(true);
  ...
  return (
    <>
      <h2 className='text-center' style={{ margin: '35px 0px', marginTop: '90px' }}>
        NewsMonkey - Top {CapitaliseFirstLetter(props.category)} Headlines
      </h2>
      {loading && <Spinner />}
      <InfiniteScroll
